...
const Swatch: React.FC<SwatchProps> = ({ className, description, isDark }) => {
  ...
  return (
    <div>
      <div
        ref={colorDivRef}
        className={`canvas-${className} h-16 rounded border p-4 text-center font-bold stroke`}
      ></div>
      <p
        className="text-sm font-bold capitalize type"
        style={{ marginBottom: '0' }}
      >
        {toTitleCase(className)}
      </p>
      <p className="text-sm uppercase type" style={{ margin: '0' }}>
        {hexColor && `${hexColor}`}
      </p>
